...
FrameBuffer::~FrameBuffer() {
    // Color buffers use the helper context during their destructors.
    m_colorbuffers.clear();
    m_windows.clear();
    m_contexts.clear();
    if (m_eglDisplay != EGL_NO_DISPLAY) {
        if (m_pbufContext != EGL_NO_CONTEXT && m_pbufSurface != EGL_NO_SURFACE)
            s_egl.eglMakeCurrent(m_eglDisplay, m_pbufSurface, m_pbufSurface, m_pbufContext);
        delete m_textureDraw;
        s_egl.eglMakeCurrent(m_eglDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
        if (m_eglContext != EGL_NO_CONTEXT) s_egl.eglDestroyContext(m_eglDisplay, m_eglContext);
        if (m_pbufContext != EGL_NO_CONTEXT) s_egl.eglDestroyContext(m_eglDisplay, m_pbufContext);
        if (m_pbufSurface != EGL_NO_SURFACE) s_egl.eglDestroySurface(m_eglDisplay, m_pbufSurface);
        if (m_eglSurface != EGL_NO_SURFACE) s_egl.eglDestroySurface(m_eglDisplay, m_eglSurface);
                                          ...
                                          GLenum p_internalFormat)
{
    ...
    if (p_width <= 0 || p_height <= 0 || p_width > 4096 || p_height > 4096) return 0;
    size_t pixels = size_t(p_width) * p_height;
    for (const auto& entry : m_colorbuffers)
        pixels += size_t(entry.second.cb->width()) * entry.second.cb->height();
    // Two RGBA textures per colorbuffer, at most 128 MiB in total.
    if (pixels > 16 * 1024 * 1024 || m_colorbuffers.size() >= 256) return 0;
                                            ...
                                            bool p_isGL2)
{
    ...
    if (m_contexts.size() >= 256) return 0;
...
HandleType FrameBuffer::createWindowSurface(int p_config, int p_width, int p_height)
{
    ...
    if (p_width <= 0 || p_height <= 0 || p_width > 4096 || p_height > 4096 || m_windows.size() >= 256) return 0;
    HandleType ret = 0;
